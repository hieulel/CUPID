...
dat_input <- as.character(c6list$V1)
dat_output <- as.character(c6list$V2)

dat_raw <- read.table(file="sample.c7.in",
                      fill = TRUE,
                      header = FALSE, skip = 3, comment.char = "(" ,
                      dec = ".", stringsAsFactors = F,nrows = 55)


# GRAB THE ORIGINAL INPUT 
# Infix 1 -13
ispecl <- dat_raw[5,1] # Print vertical profile, fixed
nn2pnt = dat_raw[7,1]; ispn=dat_raw[7,2] # CODDE, Fixed
MMDAYIN <- dat_raw[8,1]; MDAY <- dat_raw[8,2] #mdayin = 1 --> all day
LAT <- dat_raw[9,1]; LONG <- dat_raw[9,2]; STD_LONG <- dat_raw[9,3] # Lat and Long
Slope <- dat_raw[10,1]; Aspect <- dat_raw[10,2]; HRZANGLE <- dat_raw[10,3] # SA(1)

# (ispecl,nn2pnt,ispn,MMDAYIN,MDAY,LAT,LONG,STD_LONG,Slope,Aspec,HRZANGLE)

# Insoil 14 -21
# Clodsize = Index, fixed, only change resdue (residual on soil)
...
# (CLODSZ,NDSOIL,REFDPT,resdue,Z0soil,Zsoil,NLSOIL,ZLSOIL,BD,SAND,SILT,CLAY,QRTZ,PE,BX,DAK,IDOROK,DELD,DRHSFC,RHSLOP)
...
# (TIRRIG,ETSUM,QSUM,ETFIX,QFIX,ZMAX,HTSPK,ABSPK,BSPK,SPKPRS,BYANG,TDROPI)

inspnkl = c('TIRRIG','ETSUM','QSUM','ETFIX','QFIX','ZMAX','HTSPK','ABSPK','BSPK','SPKPRS','BYANG','TDROPI')
inspnkl_v = as.vector(unlist(c(TIRRIG,ETSUM,QSUM,ETFIX,QFIX,ZMAX,HTSPK,ABSPK,BSPK,SPKPRS,BYANG,TDROPI)))

# Inplant 28 - 57

g(ibidir,itassl) %=% dat_raw[25,1:2] # Ibidirectional 
ipp <- dat_raw[26,1]
nvzenp <- dat_raw[27,1]
...
NLACPY <- dat_raw[43,1]; Z_Labove <- dat_raw[43,2:(1+NLACPY)] #Number of layer above canopy & height
NLBCPY <- dat_raw[44,1]; Z_lbelow = dat_raw[44,2:(1+NLBCPY)] # Number of canopy layer below the canopy & height
...
# (ibidir,itassl,ipp,nvzenp,vwzenp,NOZENV,NOAZMV,ITOT,JMAX,JMIN,DFMIN,CLUMP,kmax,EMIS,EMISOL,factir,RSOIL,RLEAF,TLEAF,RLDEAD,TLDEAD,AVISR,BVISR,CVISR,ANIRR,BNIRR,AVIST,BVIST,CVIST,ANIRT,BNIRT,ISPHER, NALPHA, GR_ALPHA,IMUU,XMEU,XNEU,imunua,xmeuaz,xneuaz,beta0,nbeta,ROWSP,PLSPC,DMAX,ILAIUP,PLTPM2,REFHTW, ZDHCR, amfull, z0dh, dispdh,NLACPY,Z_Labove,NLBCPY,Z_lbelow,RCUT20,RSMIN,ANSTOM,RADN,TRSOPT, TRSMAX, TRSMIN, RSEXP, RSM,PSI1, PSI2, cidcai,IC3IC4, CAIR, FS, CUCOND, D1, bkh, conmin, RXCHAM,NOTEMP,TFIX,RROOT,FRSTEM,FRWTMX,PINTMX,NODYIN,LDAYIN,XTLAI,frlive,XHT,XAROT,XZLDH,XZMDH,TSOLBC,WSOLBC)

inplant <- c('ibidir','itassl','ipp','nvzenp','vwzenp','NOZENV','NOAZMV','ITOT','JMAX','JMIN','DFMIN','CLUMP','kmax','EMIS','EMISOL','factir','RSOIL','RLEAF','TLEAF','RLDEAD','TLDEAD','AVISR','BVISR','CVISR','ANIRR','BNIRR','AVIST','BVIST','CVIST','ANIRT','BNIRT','ISPHER','NALPHA','GR_ALPHA','IMUU','XMEU','XNEU','imunua','xmeuaz','xneuaz','beta0','nbeta','ROWSP','PLSPC','DMAX','ILAIUP','PLTPM2','REFHTW','ZDHCR','amfull','z0dh','dispdh','NLACPY','Z_Labove','NLBCPY','Z_lbelow','RCUT20','RSMIN','ANSTOM','RADN','TRSOPT','TRSMAX','TRSMIN','RSEXP','RSM','PSI1','PSI2','cidcai','IC3IC4','CAIR','FS','CUCOND','D1','bkh','conmin','RXCHAM','NOTEMP','TFIX','RROOT','FRSTEM','FRWTMX','PINTMX','NODYIN','LDAYIN','XTLAI','frlive','XHT','XAROT','XZLDH','XZMDH','TSOLBC','WSOLBC')

# Initc 58
WT_i <- dat_raw[55,1:NDSOIL] # Initial water content profile
# --------------------------

# Weather station data
cupid.input.weather <- read.table(file="sample.c7.in",
                          fill = TRUE,
                          header = FALSE,
                          skip = 59)

names(cupid.input.weather) <- c('year','doy','local',
